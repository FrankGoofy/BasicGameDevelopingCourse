// TextEditor.cpp : 定义应用程序的入口点。
//

#include "pch.h"
#include "framework.h"
#include "GameDevExper01.h"
#include <commdlg.h>
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_GAMEDEVEXPER01, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_GAMEDEVEXPER01));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_GAMEDEVEXPER01));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_GAMEDEVEXPER01);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hWndEdit = CreateWindowW(
		...
		WS_VISIBLE | WS_CHILD | ES_LEFT | ES_MULTILINE | ES_WANTRETURN | WS_VSCROLL | ES_AUTOVSCROLL,
	...
	case WM_COMMAND:
	{
		...
		case ID_OPEN:
			...
			if (GetOpenFileName(&ofn) == TRUE)
			{
				...
				SimpleFile simpfile;
				fileLines = simpfile.getFileLines(ofn.lpstrFile);
				...
				SetWindowText(hWndEdit, (LPCWSTR)allLines.c_str());
			}
			break;
		...
		case ID_SAVE:
			...
			//TODO：goto是一个暂时实现保存无路径文本的方案，等待改进
			if (wcslen(currentFile) == 0)
			{
				goto FLAG;
			}
		...
		{
			std::wstring  text = L"";
			SetWindowText(hWndEdit, (LPCWSTR)text.c_str());
		}
		break;
		default:
